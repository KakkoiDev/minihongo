...
def load(apkg: Path) -> dict:
    ...
    # does not register as a difference. Recorded by deck *name*, not ID:
    # minihongo randomises deck IDs on every build, and Anki merges decks by
    # name on import anyway, so the name is what a learner's collection follows.
    placement: dict[str, list[str]] = {}
    raw_placement: dict[str, list[int]] = {}
    for guid, did in connection.execute(
        "select n.guid, c.did from cards c join notes n on n.id = c.nid"
    ):
        raw_placement.setdefault(guid, []).append(did)
    ...
    for guid, dids in raw_placement.items():
        placement[guid] = sorted(decks.get(did, f"<unknown {did}>") for did in dids)
...
def compare(old: dict, new: dict) -> list[str]:
    ...
    old_names, new_names = set(old["decks"].values()), set(new["decks"].values())
    if lost_decks := old_names - new_names:
        problems.append(f"deck(s) disappeared: {sorted(lost_decks)}")
    if gained_decks := new_names - old_names:
        problems.append(f"deck(s) appeared: {sorted(gained_decks)}")
